...
/**
 TradeSeal Marketplace Contract
 Flow:
...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    function createListing(
        ...
        require(approvedTokens[paymentToken], "Not approved");
...
function updateListing(
    ...
    require(price > 0, "price=0");
    require(initialQuantity > 0, "qty=0");

    L.price = price;
    L.active = active;
    L.initialQuantity = initialQuantity;
    L.quantity = initialQuantity; // ✅ reset stock on restock

    emit ListingUpdated(listingId, price, active, initialQuantity, L.quantity);
}
    ...
    function buyerConfirmAndPay(uint256 orderId) external payable nonReentrant {
        ...
        if (O.paymentToken == address(0)) {
            require(msg.value == total, "bad");
        } else {
            require(msg.value == 0, "none");
...
function confirmDelivery(
    ...
    function sellerCancelOrder(uint256 orderId) external {
    Order storage O = orders[orderId];
    Listing storage L = listings[O.listingId];

    // Only allow cancel before funds enter escrow
    require(
        O.status == OrderStatus.Requested || O.status == OrderStatus.ShippingSet,
        "cannot cancel after escrow"
    );
    require(L.seller == msg.sender, "not seller");
...
function resolveDispute(
    ...
    function getAllDisputes() external view returns (Order[] memory) {
    uint256 count;
    for (uint256 i = 1; i <= orderCount; i++) {
        if (
    ...
    /// Return all orders currently in Disputed status
    function getAllOpenDisputes() external view returns (Order[] memory) {
        uint256 count;
        for (uint256 i = 1; i <= orderCount; i++) {
            if (orders[i].status == OrderStatus.Disputed) count++;
        }
        Order[] memory arr = new Order[](count);
        uint256 idx = 0;
        for (uint256 i = 1; i <= orderCount; i++) {
            if (orders[i].status == OrderStatus.Disputed) {
                arr[idx] = orders[i];
                idx++;
            }
        }
    ...
    function getResolvedDisputes() external view returns (Order[] memory) {
    uint256 count = 0;
    for (uint256 i = 1; i <= orderCount; i++) {
        if (orders[i].status == OrderStatus.DisputeResolved) {
            count++;
        }
    }
